...
## Docker
Сперва, конечно, нужно скачать докер на ваш компьютер.
Вы можете расскоментировать строчку `# FROM texlive/texlive:latest-small` и использовать только её, но дело в том, что обновления контейнера происходят довольно часто, поэтому рекомендуется зафиксировать его версию, используя хеш. Т.е. вам следует сначала использовать `texlive/texlive:latest-small`, собрать Dockerfile, а затем найти хеш (`docker images --digests`) и заменить строчку `FROM texlive/texlive:latest-small` на строчку вида `texlive/texlive@sha256:8aa671eb1ad026bff4103bd2f5744eab9b4f33faab8a6bcd673590e2128720e2`. **ВНИМАНИЕ! У вас будет другая строчка! Текущая приведена в качестве примера!!!**

Примечательно, что если вы используете расширение Docker, вам следует отключить настройку `containers.images.checkForOutdatedImages` в нем. Иначе он будет пытаться найти обновления данного хэша (`texlive/texlive@sha256:8aa67...`) и сборка у вас проходить не будет.
