...
# Docker
## Ошибки
Часто так бывает, что git неправильно подхватывает Dockerfile. Убедитесь, что он называется `Dockerfile` (буква `f` должна быть маленькой)!

## Ядерка
Если вы добавили что-то в проект, улучшили что-то, а одна и та же ошибка все возникает и возникает, возможно вам просто нужно СБРОСИТЬ ЯДЕРКУ НА ЭТОТ ДОКЕР. Бывает, глючит. Оставлю это здесь. ВНИМАНИЕ! ОЧЕНЬ ОПАСНАЯ КОМАНДА! ОНА УДАЛЯЕТ ВООБЩЕ ВСЕ ДОКЕР КОНТЕЙНЕРЫ! Не удалите свои личные или те, что вы используете! Вы должны знать, что используете! (Если же вы ставили докер только ради данного проекта, то можете спокойно ее применять.)
```
docker system prune -a --volumes
```
Не забудьте перезапустить Docker Desktop правой кнопкой по иконке в трее -> Quit Docker Desktop, не забудьте перезапустить снова. Потом можете просто как обычно начать компилировать отчет.

## Еще полезные команды Docker
`docker exec -it latex-daemon bash` - попасть внутрь контейнера
