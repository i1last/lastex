...
Пример расчета полных потерь энергии для $f = \pv[.0]{r['f_loss'][0]}$ Гц ($S_П = \pv[.1]{r['S_P'][0]}$ мм$^2$), учитывая, что у нас нанокристаллический сплав ($d = \pv[.0]{p['prms']['d']} кг/м^2$):
\begin{equation}
    Э = \frac{S_П h_1 b_1}{100 d} = \frac{\pv[.1]{r['S_P'][0]} \cdot \pv[.2]{r['h1']} \cdot \pv[.4]{r['b1']}}{100 \cdot \pv[.0]{p['prms']['d']}} = \pv[-6.4]{r['E'][0]} \text{ мкДж/кг}
\end{equation}

Экстраполяция линейной зависимости $Э(f)$ к $f=0$ дает значение потерь на гистерезис: $Э_Г = \pv[-6.4]{r['E_G']}$ мкДж/кг.

Пример расчета потерь на вихревые токи и мощностей для $f = \pv[.0]{r['f_loss'][0]}$ Гц:
\begin{equation}
    Э_{ВТ} = Э - Э_Г = \pv[-6.4]{r['E'][0]} - \pv[-6.4]{r['E_G']} = \pv[-6.4]{r['E_BT'][0]} \text{ мкДж/кг}
\end{equation}
\begin{equation}
    P_Г = Э_Г f = \pv[-6.4]{r['E_G']} \cdot \pv[.0]{r['f_loss'][0]} = \pv[-6.2]{r['P_G'][0]} \text{ мкВт/кг}
\end{equation}
\begin{equation}
    P_{ВТ} = Э_{ВТ} f = \pv[-6.4]{r['E_BT'][0]} \cdot \pv[.0]{r['f_loss'][0]} = \pv[-6.2]{r['P_BT'][0]} \text{ мкВт/кг}
\end{equation}
...
\begin{longtblr}[
    ...
    $f$, Гц & $S_П$, мм$^2$ & $Э$, мкДж/кг & $Э_Г$, мкДж/кг & $Э_{ВТ}$, мкДж/кг & $P_Г$, мкВт/кг & $P_{ВТ}$, мкВт/кг \\
    \expanded{\pt*[.0, .2, -6.4, -6.4, -6.4, -6.2, -6.2]{*r['tab3']}}
\end{longtblr}

\begin{figure}[H]
    \centering
    \input{figs/plot_losses.pgf}
...
\begin{itemize}
    ...
    \item \textit{Магнитные потери.} Анализ петель гистерезиса показал, что с увеличением частоты потери энергии линейно возрастают. Методом экстраполяции к нулевой частоте определены потери на гистерезис $Э_Г \approx 6.42$ мкДж/кг.
