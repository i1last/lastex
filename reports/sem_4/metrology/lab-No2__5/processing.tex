...
Относительная погрешность измерения напряжения $\delta_{U_0}$:
\begin{equation}
    \delta_{U_0} = \frac{\Delta U_0}{U_0} \cdot 100\% = \frac{\directlua{tex.print(Delta_instr)}}{\directlua{tex.print(U0_s)}} \cdot 100\% = \directlua{tex.print(round(rel_delta_U0, 2))} \%
\end{equation}
...
\begin{equation}
    \Delta I = I \cdot \frac{\delta_I}{100} = \directlua{tex.print(round(I_s, 5))} \cdot \frac{\directlua{tex.print(round(delta_I_rel, 3))}}{100} = \directlua{tex.print(round(abs_delta_I, 6))} \text{ А}
\end{equation}

Результат измерения тока:
\begin{equation*}
    { I_x = (\directlua{tex.print(round(I_s, 3))} \pm \directlua{tex.print(round(abs_delta_I, 3))}) \text{ А} }
\end{equation*}
...
\begin{luacode*}
    function t89()
        for i = 1, #U1_arr do
            ...
            tex.print(string.format("%d & %.3f & %.3f & %.6f \\\\", i, ui, diff, diff_sq))
        end
    end
\end{luacode*}
...
Результат:
\begin{equation*}
    { I_x = (\directlua{tex.print(round(I_mean, 5))} \pm \directlua{tex.print(round(delta_I_multi, 5))}) \text{ А} }
\end{equation*}
\begin{equation*}
...
Результат:
\begin{equation*}
    { P_{Mx} = (\directlua{tex.print(round(P_multi_mean, 4))} \pm \directlua{tex.print(round(delta_P_multi, 4))}) \text{ Вт}, \quad P = \directlua{tex.print(prob_P)} }
\end{equation*}
\begin{equation*}
...
\begin{enumerate}
    \item В ходе прямых и косвенных однократных измерений установлено влияние отношения измеряемой величины к точноти измерений прибора. Относительная погрешность прямого измерения напряжения составила $\delta_U = \directlua{tex.print(round(rel_delta_U, 3))}\%$, при этом погрешность косвенного измерения тока достигла $\delta_I = \directlua{tex.print(round(delta_I_rel, 3))}\%$. Данное расхождение вызвано тем, что измеряемое падение напряжения на образцовом резисторе ($U_0 = \directlua{tex.print(U0_s*1000)} \text{ мВ}$) соизмеримо с абсолютной инструментальной погрешностью вольтметра ($\Delta_V = \directlua{tex.print(abs_delta_U0 * 1000)} \text{ мВ}$).
    
    \item При обработке многократных измерений установлено, что наличие случайных погрешностей существенно увеличивает разброс результатов. При сильном зашумлении сигнала (где относительная погрешность измерения тока достигает $\delta_I = \directlua{tex.print(round(delta_I_multi / I_mean * 100, 2))}\%$) использование статистических методов обработки позволяет корректно определить доверительный интервал для истинного значения измеряемой величины с заданной вероятностью $P = 0.95 \%$.
    
    \item При косвенных многократных измерениях мощности расчет эффективного числа степеней свободы ($f_{\text{эф}} = \directlua{tex.print(round(f_eff, 0))}$) подтвердил, что итоговая дисперсия результата формируется аргументами неравномерно. Анализ дисперсионных слагаемых показал, что определяющий вклад в погрешность измерения мощности вносит высокая дисперсия измерения тока.
    
    \item Оформление конечных результатов подтвердило физическую бессмысленность сохранения избыточных значащих цифр в условиях высокой дисперсии. Метрологическое округление доверительных интервалов до одной значащей цифры строго ограничивает разрядность среднего значения, что наглядно отражено в итоговой записи для мощности:
    $$P_{Mx} = (\directlua{tex.print(round(P_multi_mean, 2))} \pm \directlua{tex.print(round(delta_P_multi, 2))}) \text{ Вт}$$
\end{enumerate}