...
\begin{equation}
    H(s) = \frac{\val[3]{tf.N2}s^2 + \val[3]{tf.N0}}{\val[3]{tf.D3}s^3 + \val[3]{tf.D2}s^2 + \val[3]{tf.D1}s + \val[3]{tf.D0}}
    \label{form:hs}
\end{equation}

\subsection{Нули и полюсы передаточной функции}

Нули функции $H(s)$ определяются из уравнения $\val[3]{tf.N2}s^2 + \val[3]{tf.N0} = 0$:
\begin{equation}
    s_{01,02} = \pm j \val[2]{tf.z1_im}
    \label{form:zeros}
\end{equation}
...
\begin{align}
    s_1 &= \val[2]{tf.p1_re} \\
    s_{2,3} &= \val[2]{tf.p2_re} \pm j \val[2]{tf.p2_im}
    \label{form:poles}
\end{align}

\begin{figure}[h]
    \centering
    \input{figs/pz_map.pgf}
...
\begin{equation}
    t_{\text{пп}} \approx \frac{3}{\min_k |\text{Re}\{s_k\}|} = \frac{3}{|\val[2]{tf.p2_re}|} \approx \val[2]{tf.t_pp} \text{ с}
\end{equation}
