...
Для нахождения передаточной функции $H(s) = {I_{R2}(s)}/{I_1(s)}$ применяется метод пропорциональных величин. Цепь представляется операторной схемой замещения при нулевых начальных условиях.
...
\begin{align}
    s_1 &= \pv[.3]{tf['p1_re']} \\
    s_{2,3} &= \pv[.3]{tf['p2_re']} \pm j \pv[.3]{tf['p2_im']}
    \label{form:poles}
\end{align}
...
\begin{equation}
    t_{\text{пп}} \approx \frac{3}{\min_k |\text{Re}\{s_k\}|} = \frac{3}{|\pv[.3]{tf['p2_re']}|} \approx \pv[.2]{tf['t_pp']} \text{ с}
\end{equation}